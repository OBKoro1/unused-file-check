...
class UserConfigHandle {
    ...
    checkConfigHandle() {
        if (this.userConfig.checkFileItemPath) {
            this.userConfig.checkFileItemPath = pathAbsolute(
                this.userConfig.checkFileItemPath,
                'checkFileItemPath',
                'Directory'
            )
        }
        if (this.userConfig.outputDirectoryPath) {
            this.userConfig.outputDirectoryPath = pathAbsolute(
                this.userConfig.outputDirectoryPath,
                'outputDirectoryPath',
                'Directory'
            )
        }
    ...
    itemHandle(item, index) {
        if (!checkUnusedFileTypeOptions[item.checkUnusedFileType])
            item.checkUnusedFileType = checkUnusedFileTypeOptions.all
    ...
    checkOptionsArrItem(item) {
        ...
        if (
            ...
            if (!item.command) {
                console.log(
                    chalk.red(
                        `配置出错: 当checkUnusedFileType是${checkUnusedFileType}时,检测配置checkOptionArr中没有command`
                    )
                )
                process.exit(1)
            }
        ...
        if (
            ...
            if (!item.astEntryFile) {
                console.log(
                    chalk.red(
                        `配置出错: 当checkUnusedFileType是${checkUnusedFileType}时,检测配置checkOptionArr中没有astEntryFile`
                    )
                )
                process.exit(1)
            }
