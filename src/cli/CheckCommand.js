...
class CheckCommand {
  ...
  runCommandArr () {
    for (const [index, item] of this.checkOptionArr.entries()) {
      ...
      if (checkUnusedFileType === checkUnusedFileTypeOptions.all || checkUnusedFileType === checkUnusedFileTypeOptions.webpack) {
        console.log(`执行webpack打包命令: ${command}`)
        this.runBuild(envCommand)
      }
      if (checkUnusedFileType === checkUnusedFileTypeOptions.all || checkUnusedFileType === checkUnusedFileTypeOptions.ast) {
  ...
  runBuild (command) {
    ...
    if (res.code !== 0) {
      console.log(
        chalk.red('Error 命令执行失败: ', command)
      )
      shelljs.exit(1)
    }
    console.log(
      chalk.green('success 命令执行成功: ', command)
    )
  }
}
